...
static void osprd_process_request(osprd_info_t *d, struct request *req)
{
	...
	// Bad request.
		...
		// Bad request.  End bad request and return immediately.
		end_request(req,0);
		return;
	}
		...
		unsigned int cmd, unsigned long arg)
{
	...
	if (cmd == OSPRDIOCACQUIRE) {
		...
		// be protected by a spinlock; which ones?)
			...
			// just let the next ticket have a chance to run
			// since this task was interrupted by a signal.
		...
		// Your code here (instead of the next two lines).
                ...
                if(filp_writable) {
                    ...
                    if(!read_lock_condition(d))
                        ...
                        //On success, read_lock_condition will not release the spin lock. Release it now
                        osp_spin_unlock(&d->mutex);
                    }
                }
